...
# ===================== 只加这2个工具函数（用于精准标签计算） =====================
def _safe_duration(x):
    return max(float(x), 1e-6)  # 防止除以0


def _interval_overlap(a_l, a_r, b_l, b_r):
    return max(0.0, min(a_r, b_r) - max(a_l, b_l))  # 区间重叠计算
...
def make_dataset(split_file, split, root, num_classes=157):
    ...
    for vid in tqdm(data.keys()):
        ...
        # ===================== 安全计算时长 & FPS =====================
        duration = _safe_duration(data[vid]['duration'])
        ...
        for ann in data[vid]['actions']:
            ...
            # Use the Charades temporal-overlap rule for frame labels.
            for fr in range(num_feat):
                frame_l = fr / fps
                frame_r = (fr + 1) / fps
                if _interval_overlap(frame_l, frame_r, t_start, t_end) > 0:
                    label[fr, cls] = 1.0
            # ========================================================================
        ...
        dataset.append(
            (vid, label, data[vid]['duration'], [hmap, num_action, np.asarray(center_loc), np.asarray(action_lengths)]))
        i += 1

    return dataset


# ===================== 以下所有代码 100% 完全原样，一行没改 =====================
